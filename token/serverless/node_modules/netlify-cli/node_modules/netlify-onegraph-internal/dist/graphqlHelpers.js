    ...
    var objectHelper = function (type, selectionSet) {
        ...
        selectionSet.selections.forEach(function (selection) {
            ...
            if (selection.kind === graphql_1.Kind.FRAGMENT_SPREAD) {
                ...
                if (!field) {
                    console.warn("Could not find field", name_2, "on", parentNamedType.name, "among", 
                    // @ts-ignore
                    Object.keys(parentNamedType.getFields()));
                    return;
                }
...
var gatherHardcodedValues = function (query) {
    var parsedQuery;
    try {
        ...
        var isHardcodedValueNode_1 = function (node) {
            var isHardcodedValue = node.value &&
                (node.value.kind === graphql_1.Kind.STRING ||
                    node.value.kind === graphql_1.Kind.INT ||
                    node.value.kind === graphql_1.Kind.FLOAT);
        ...
        var hardCodedValueExtractor = function (node) {
            ...
            if (isHardcodedValue) {
                ...
                else if (node.value.kind === graphql_1.Kind.INT) {
                    nodeValue = node.value.value;
                }
                else if (node.value.kind === graphql_1.Kind.FLOAT) {
                    nodeValue = node.value.value;
                }
                if (nodeValue) {
...
var extractPersistableOperation = function (doc, operationDefinition) {
    ...
    var fragmentExtractor = function (node) {
        ...
        var fragmentDefinition = doc.definitions.find(function (def) {
            return def.kind === graphql_1.Kind.FRAGMENT_DEFINITION && def.name.value === fragmentName;
        });
        if (fragmentDefinition) {
            fragments.add(fragmentDefinition);
        }
        else {
            console.warn("Could not find fragment definition for referenced fragment: ", fragmentName);
        }
        return node;
    };
    var newOperation = (0, graphql_1.visit)(operationDefinition, {
    ...
    var fullDoc = __spreadArray([(0, graphql_1.print)(newOperation)], fragmentStrings, true).join("\n\n");
    return fullDoc;
};
