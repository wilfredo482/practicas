...
exports.HtpasswdHashAlgorithm = void 0;
...
async function verifyPassword(passwd, hash) {
  if (hash.match(/^\$2([aby])\$/)) {
    return new Promise((resolve, reject) => _bcryptjs.default.compare(passwd, hash, (error, result) => error ? reject(error) : resolve(result)));
...
function generateHtpasswdLine(user, passwd, hashConfig) {
  let hash;

  switch (hashConfig.algorithm) {
    case HtpasswdHashAlgorithm.bcrypt:
      hash = _bcryptjs.default.hashSync(passwd, hashConfig.rounds);
      break;
...
function addUserToHTPasswd(body, user, passwd, hashConfig) {
